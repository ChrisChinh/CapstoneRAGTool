...
from azure_client_copy import AzureClient
import threading
import re
import logging
...
class GUI(Tk):
    ...
    def _build_bottom_bar(self):
        ...
        self.url_btn = Button(buttons_wrap, text="Load URL", width=12, command=self.on_url_upload)
        self.url_btn.pack(side=LEFT, padx=8)
    ...
    def on_url_upload(self):
        url = simpledialog.askstring("Input URL", "Please paste the documentation URL:")
        # Check if the user clicked 'Cancel' or left it blank
        if url:
            url = url.strip()
            try:
                # Show a temporary status so the user knows it's working
                print(f"Scraping: {url}")
                
                # Call your existing search_url function
                self.ix.search_url(url)
                
                messagebox.showinfo("Success", "Website content has been added!")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to scrape URL: {e}")
        else:
            print("User cancelled the URL input.")
